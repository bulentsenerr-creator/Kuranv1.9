/* Kur'an PWA v1.9
   ...
   v1.9 ekleri:
   - Paralel indirme (1-3 job) + öncelik (⬆⬇)
   - Tüm cüzleri kuyruğa ekle + toplu iptal

   Not: Büyük indirmeler depolamayı doldurabilir. ETA/hız yaklaşık hesaplanır.
*/
...
const DB_VER = 8; // v1.9: wifiOnly + juz open meta // v1.9
...
async function init(){
  ...
  document.body.classList.toggle('hide-tr', store.get('hideTr', false));
  document.body.classList.toggle('hide-ar', store.get('hideAr', false));
  el.hideTr.checked = store.get('hideTr', false);
  el.hideAr.checked = store.get('hideAr', false);

  // init manager controls
  if (el.autoResumeJobs) el.autoResumeJobs.checked = !!state.autoResumeJobs;
  if (el.maxConcurrentDownloads) el.maxConcurrentDownloads.value = String(state.maxConcurrentDownloads || 2);
  if (el.wifiOnlyDownloads) el.wifiOnlyDownloads.checked = !!state.wifiOnlyDownloads;
  ...
  if (state.autoResumeJobs && navigator.onLine){
    if (warnIfNotWifi()) await startNextJobIfIdle();
  }
...
function wireUI(){
  ...
  el.pageSelect.addEventListener('change', async (e)=>{
  ...
  el.juzSelect.addEventListener('change', async (e)=>{
    state.selectedJuz = Number(e.target.value);
    store.set('selectedJuz', state.selectedJuz);
    await syncJuzAudioButtons();
  });
  ...
  // manager controls
  el.textPackFilter.addEventListener('change', async (e)=>{
    state.textPackFilter = e.target.value;
    store.set('textPackFilter', state.textPackFilter);
...
function shiftPage(delta){
  ...
  el.pageSelect.value=String(next);
}
...
function getPages(){
  const pages=[...new Set((state.data.arabicAyahs||[]).map(a=>a.page).filter(Boolean))].sort((a,b)=>a-b);
  return pages;
}
  ...
  for (let i=0;i<urls.length;i++){
    ...
    try{
      ...
      if (!existing){
        ...
        if (res.ok){ await cache.put(req, res.clone()); ok++; }
      } else ok++;
    }catch{}
  }

  el.dlBar.style.width='100%';
  el.dlText.textContent=`Tamamlandı. Offline ses hazır: ${ok}/${urls.length}`;
...
async function downloadCurrentSurahTextOffline(){
  ...
  try{
    ...
    el.txtText.textContent='Hata: Metin+Meal indirilemedi.';
  }
...
function isWifiConnection(){
  ...
  if (type.includes('wifi')) return true;
  if (type.includes('ethernet')) return true;
  if (type.includes('none')) return false;
  // cellular / 4g / 5g / 3g etc
  return false;
}

function warnIfNotWifi(){
  const isWifi = isWifiConnection();
  if (isWifi === null) return true; // cannot detect
...
async function normalizeJobsOnStartup(){
  ...
  if (changed) await renderJobList();
...
async function startBulkAudioDownload(){
  ...
  const job={
    id, type:'bulkAudio', reciter,
    priority: 10,
    status:'running',
    progress:{ surah:1, ayahIndex:0, doneAyahs:0, totalAyahs:6236, bytesDone:0, startedAt:Date.now() },
...
async function startJuzAudioDownload(juz){
  ...
  const job={
    id, type:'juzAudio', reciter, juz,
    priority: 20 + Number(juz||0),
    status:'running',
    progress:{ ayahIndex:0, doneAyahs:0, totalAyahs:0, bytesDone:0, startedAt:Date.now() },
    createdAt:Date.now(), updatedAt:Date.now(),
    message:`Başlıyor… (Cüz ${juz})`
  };
  await dbPut('jobs', job);
  await syncJuzAudioButtons();
  await runJobRunner();
...
async function enqueueAllJuzAudioJobs(){
  ...
  for (let j=1; j<=30; j++){
    ...
    const job={
      id, type:'juzAudio', reciter, juz:j,
      priority: 20+j,
      status:'paused',
      progress:{ ayahIndex:0, doneAyahs:0, totalAyahs:0, bytesDone:0, startedAt:null },
      createdAt:Date.now(), updatedAt:Date.now(),
      message:`Kuyruğa eklendi (Cüz ${j}).`
    };
    await dbPut('jobs', job);
  }
...
async function resumeJob(id){
  const job=await dbGet('jobs', id);
  if (!job) return;
  if (job.status==='completed') return alert('Bu indirme zaten tamamlanmış.');
  job.status='running';
  job.updatedAt=Date.now();
...
async function runJobRunner(){
  ...
  try{
    while (true){
      ...
      await syncBulkAudioButtons();
      await syncJuzAudioButtons();
      if (el.offDialog.open) { await renderJobList(); await updateAllJuzSummary(); }
...
// Returns true if any download progress was made
async function processBulkAudioJob(job, stepLimit=6){
  ...
  for (let s=job.progress.surah||1; s<=114; s++){
    ...
    let surahData;
    try{
      const res=await fetchJSON(`${API}/surah/${s}/${reciter}`);
      surahData=res.data;
    } catch {
      fresh.status='paused';
      fresh.message='Ağ hatası. Duraklatıldı.';
      fresh.updatedAt=Date.now();
      await dbPut('jobs', fresh);
      return steps>0;
    }
...
async function processJuzAudioJob(job, stepLimit=10){
  ...
  let juzData;
  try{
    const res=await fetchJSON(`${API}/juz/${juz}/${reciter}`);
    juzData=res.data;
  } catch {
    job.status='paused';
    job.message='Ağ hatası. Duraklatıldı.';
    job.updatedAt=Date.now();
    await dbPut('jobs', job);
    return steps>0;
  }
  ...
  // cache first ref for 'Aç'
  try{
    if (ayahs[0]?.surah?.number && ayahs[0]?.numberInSurah){
      await dbPut('kv', { key: `juzFirstRef:${juz}`, value: { surah: ayahs[0].surah.number, ayah: ayahs[0].numberInSurah } });
    }
  }catch{}
...
// ---------------- UI sync for job buttons (bulk & juz) ----------------
...
async function syncBulkAudioButtons(){
  ...
  if (job.status==='running'){
    el.btnPauseAllAudio.disabled=false;
    el.btnCancelAllAudio.disabled=false;
  } else if (job.status==='paused'){
    el.btnResumeAllAudio.disabled=false;
    el.btnCancelAllAudio.disabled=false;
...
async function syncJuzAudioButtons(){
  ...
  if (job.status==='running'){
    el.btnPauseJuzAudio.disabled=false;
    el.btnCancelJuzAudio.disabled=false;
  } else if (job.status==='paused'){
    el.btnResumeJuzAudio.disabled=false;
    el.btnCancelJuzAudio.disabled=false;
  } else if (job.status==='cancelled'){
    el.btnResumeJuzAudio.disabled=false;
  }
...
async function updateOfflineIndicators(){
  const hasAudio=(await caches.keys()).includes(audioCacheName());
...
async function updateAllJuzSummary(){
  if (!el.allJuzSummary) return;
  const reciter = state.audioEdition;
  const jobs = (await dbGetAll('jobs')).filter(j => j.type==='juzAudio' && j.reciter===reciter);
  if (!jobs.length){
    el.allJuzSummary.hidden = true;
    el.allJuzSummary.textContent = '';
    return;
  }
...
async function getJuzFirstRef(juz){
  const key = `juzFirstRef:${juz}`;
  const cached = await dbGet('kv', key);
  if (cached?.value) return cached.value;
  // try fetch minimal
  try{
    const res = await fetchJSON(`${API}/juz/${juz}/${state.arabicEdition}?offset=0&limit=1`);
    const ayah = res.data?.ayahs?.[0];
    if (ayah && ayah.surah?.number && ayah.numberInSurah){
      const value = { surah: ayah.surah.number, ayah: ayah.numberInSurah };
...
async function renderOfflineLists(){
  ...
  if (!audioKeys.length){
    ...
    const renderGroup=(title, keys)=>{
      ...
      return `<div class="ayah"><div class="tr muted"><b>${escapeHTML(title)}</b></div></div>` + keys.map(k=>{
        ...
        return `<div class="ayah"><div class="ayahHead"><div class="badge">${escapeHTML(k)}</div><div class="tools"><span class="muted tiny">${sz!=null?formatBytes(sz):'Boyut: ?'}</span><button class="btn small" data-audgo="${escapeHTML(k)}">Aç</button><button class="btn small ghost" data-auddel="${escapeHTML(k)}">Sil</button></div></div><div class="tr muted">${escapeHTML(parsed.label)}</div></div>`;
      }).join('');
    };

    el.offAudioList.innerHTML = renderGroup('Sure Ses Paketleri', surah) + renderGroup('Cüz Ses Paketleri', juz) + renderGroup('Diğer', other);

    el.offAudioList.querySelectorAll('[data-audgo]').forEach(btn=>btn.addEventListener('click', async()=>{
      const k=btn.dataset.audgo;
      const m1=/^quran-offline-.+-s(\d+)$/.exec(k);
      if (m1){ await goToAyahId(`${Number(m1[1])}:1`, true); el.offDialog.close(); return; }
      const m2=/^quran-offline-.+-j(\d+)$/.exec(k);
      if (m2){
        const ref = await getJuzFirstRef(Number(m2[1]));
        if (ref){ await goToAyahId(`${ref.surah}:${ref.ayah}`, true); el.offDialog.close(); return; }
        alert('Cüz için ilk ayet bilgisi alınamadı (offline olabilir).');
        return;
      }
      alert('Bu paket için doğrudan açma desteklenmiyor.');
    }));
  ...
  if (!packs.length){
    ...
    el.offTextList.innerHTML = packs.map(p=>`<div class="ayah"><div class="ayahHead"><div class="badge">Sure ${p.surah} • ${escapeHTML(p.translationEdition)}</div><div class="tools"><span class="muted tiny">${p.bytesApprox?formatBytes(p.bytesApprox):'Boyut: ?'}</span><button class="btn small" data-txtgo="${p.surah}">Aç</button><button class="btn small ghost" data-txtdel="${escapeHTML(p.id)}">Sil</button></div></div><div class="tr muted">${escapeHTML(p.id)}</div></div>`).join('');
    el.offTextList.querySelectorAll('[data-txtgo]').forEach(btn=>btn.addEventListener('click', async()=>{
      await goToAyahId(`${btn.dataset.txtgo}:1`, true);
      el.offDialog.close();
    }));
...
async function renderJobList(){
  ...
  if (!jobs.length){
    el.jobList.innerHTML = `<div class="ayah"><div class="tr">Kuyruk boş.</div></div>`;
    return;
  }
...
async function refreshSizeSummary(force){
  ...
  if (cached && !force){
    ...
    el.sizeSummary.textContent=cached.text;
    return;
  }
...
async function refreshQuotaSummary(force){
  ...
  if (cached && !force){
    ...
    el.quotaSummary.textContent=cached.text;
    return;
  }
