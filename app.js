/* Kur'an PWA v1.9.1
   v1.9 özellikleri eksiksiz + OFFLINE SES düzeltmesi:
   - Cache key normalizasyonu (cache.put/match URL string)
   - Service Worker audio match genişletmesi (sw.js)
   - Offline çalmada blob fallback (caches.match -> blob -> objectURL)

   Android Chrome hedef: offline dinlemede "ses çıkmıyor" sorunu.
*/
...
async function init(){
  ...
  document.body.classList.toggle('hide-tr', el.hideTr.checked);
  document.body.classList.toggle('hide-ar', el.hideAr.checked);
...
function isWifiConnection(){
  ...
  if (type.includes('wifi') || type.includes('ethernet')) return true;
...
function onPlayClicked(key){
  ...
  state.audio.onerror = async () => {
    if (!navigator.onLine){
      const ok = await playFromCacheFallback(item.audio);
      if (!ok) alert('Offline ses bulunamadı. Önce bu sure/cüz için “Ses indir” yapın.');
    }
  };
  state.audio.play().catch(async ()=>{
    if (!navigator.onLine){
      const ok = await playFromCacheFallback(item.audio);
      if (!ok) alert('Offline ses bulunamadı. Önce bu sure/cüz için “Ses indir” yapın.');
    }
...
function playAudio(url){
  return new Promise((resolve)=>{
    try{
      ...
      state.audio.onerror=async ()=>{
        if (!navigator.onLine) await playFromCacheFallback(url);
        resolve();
      };
      state.audio.src=url;
      const p=state.audio.play();
      if (p && p.catch) p.catch(async ()=>{
        if (!navigator.onLine) await playFromCacheFallback(url);
        resolve();
      });
    }catch{ resolve(); }
...
// ---------- Offline: per-surah audio (key normalization) ----------
...
// ---------- v1.9 features: jobs, parallel, priority, juz open ----------
...
async function getJuzFirstRef(juz){
  ...
  try{
    ...
    if (ayah && ayah.surah?.number && ayah.numberInSurah){
      const v={ surah: ayah.surah.number, ayah: ayah.numberInSurah };
      await dbPut('kv', { key, value: v });
      return v;
    }
  }catch{}
  return null;
...
async function estimateCacheSizeBytes(cacheName){
  try{
    ...
    for (let i=0;i<reqs.length;i++){
      ...
      if (cl) sum+=Number(cl);
      else { const buf=await res.clone().arrayBuffer(); sum+=buf.byteLength; }
