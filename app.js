/* Kur'an PWA v1.9.2
   Önceki tüm özellikler korunmuştur.

   Düzeltme: Offline sesleri "indirmiyor" sorunu
   - CORS başarısızsa no-cors ile indir (opaque kabul)
   - res.ok yerine (res.ok || res.type==='opaque') cache.put
   - Wi-Fi tespiti: kesin değilse null => engelleme yok
   - Secure context kontrolü: HTTPS/kurulu PWA değilse kullanıcıya uyarı
*/
...
const el={
  ...
  // modal close buttons exist in HTML but not referenced above (to keep minimal)
};
...
function checkSecureContext(){
  ...
  if (!ok){
    ...
    el.secureWarn.textContent = '⚠️ Offline indirme çalışmayabilir: Bu sayfa güvenli bağlamda değil. Android Chrome’da offline için HTTPS üzerinden yayınlayın veya PWA olarak kurun.';
  } else {
...
function isWifiConnection(){
  ...
  if (!conn) return null;
  // Prefer conn.type if present; effectiveType is NOT reliable for Wi-Fi detection.
  if (conn.type){
...
function warnIfNotWifi(){
  ...
  if (isWifi===null) return true; // cannot detect reliably
...
async function fetchForCache(url){
  // Try CORS first; if it fails, fall back to no-cors (opaque).
  try {
...
async function playFromCacheFallback(url){
  try{
    ...
    if (!res) return false;
    // opaque response might throw on blob in some cases; catch
    let blob;
...
function onPlayClicked(key){
  ...
  state.audio.onerror = async ()=>{
    if (!navigator.onLine){
      await playFromCacheFallback(item.audio);
    }
  };
...
async function downloadCurrentSurahAudioOffline(){
  ...
  el.dlText.textContent=`Tamamlandı. Offline ses: ${ok}/${urls.length}` + (fail?` • Hata: ${fail}`:'');
...
async function processBulkAudioJob(job, stepLimit=6){
  ...
  for (let s=job.progress.surah||1; s<=114; s++){
    ...
    try{ surahData=(await fetchJSON(`${API}/surah/${s}/${reciter}`)).data; }
      ...
      fresh.status='paused'; fresh.message='Ağ hatası. Duraklatıldı.'; fresh.updatedAt=Date.now();
      await dbPut('jobs', fresh); return steps>0;
    }
...
async function processJuzAudioJob(job, stepLimit=10){
  ...
  try{ juzData=(await fetchJSON(`${API}/juz/${juz}/${reciter}`)).data; }
    ...
    job.status='paused'; job.message='Ağ hatası. Duraklatıldı.'; job.updatedAt=Date.now();
    await dbPut('jobs', job); return steps>0;
  }
